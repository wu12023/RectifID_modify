...
pipe = StableDiffusionPipeline.from_pretrained("./models/perflow-sd15-dreamshaper", safety_checker=None, torch_dtype=torch.float16, local_file_only = True)
...
def callback(self, i, t, callback_kwargs):
    print("callback_kwargs['latents'].shape",callback_kwargs['latents'].shape)
    ...
    print("type(latents_e):",type(latents_e))
...
for epoch in tqdm(range(61)):
    ...
    print((latents0+latents[:1]-latents[:1].detach()).shape)
    ...
    t4 = time.time()
    print(latents.grad)
    print("latents.grad.shape",latents.grad.shape)
